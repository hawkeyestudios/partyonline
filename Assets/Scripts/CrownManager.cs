...
public class CrownManager : MonoBehaviourPunCallbacks
{
    ...
    private float GetPlayerScore(Player player)
    {
        ...
        {
            // Ensure that the score is of type float
            if (score is float floatScore)
            {
                return floatScore;
            }
            else if (score is int intScore)
            {
                // In case it was stored as an integer, convert it to float
                return (float)intScore;
            }
            else
            {
                Debug.LogError($"Invalid score type for player {player.NickName}. Score: {score}");
            }
        }
        return 0f;
    }

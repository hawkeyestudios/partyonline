...
handlers.AddFriendRequest = function (args, context) {
    var senderId = currentPlayerId;
    var recipientId = args.RecipientId;

    // Fetch the current pending friend requests
    var recipientData = server.GetUserInternalData({
        PlayFabId: recipientId,
        Keys: ["PendingFriendRequests"]
    });

    var pendingRequests = JSON.parse(recipientData.Data.PendingFriendRequests || "[]");

    // Add the new request to the pending list
    pendingRequests.push({ SenderId: senderId, Timestamp: Date.now() });

    var updateData = server.UpdateUserInternalData({
        PlayFabId: recipientId,
        Data: {
            PendingFriendRequests: JSON.stringify(pendingRequests)
        }
    });
    ...
    server.SendPushNotification({
        ...
        Message: "You have a new friend request from " + senderId
    });
...
handlers.AcceptFriendRequest = function (args, context) {
    var recipientId = currentPlayerId;
    ...
    // Add the recipient as a friend for the sender
    server.AddFriend({
